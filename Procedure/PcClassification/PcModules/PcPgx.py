...
class PcChemotherapy(PcBaseclassify):
	...
	def read_database(self,databaseFile):
		array_db = db.get_table(databaseFile, "Pgx").get()
		table = pd.DataFrame(array_db)
		...
		for i in range(0, nrows):
			...
			毒性 = array[head.index("毒性")]
			有效性 = array[head.index("有效性")]
		...
		array_db = db.get_table(databaseFile, "rs").get()
		table = pd.DataFrame(array_db)
