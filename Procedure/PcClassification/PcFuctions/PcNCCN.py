        ...
        for i in dict_1:
            if dict_1[i]["SYMBOL"] =="EGFR":
                if dict_1[i]["HGVSp_Short"] == "p.L858R":
                    ...
                    _dict[i] = get_info(db,"EGFR","20号外显子插入")
            elif dict_1[i]["SYMBOL"] == "KRAS" and dict_1[i]["HGVSp_Short"] == "p.G12C":
                _dict[i] = get_info(db,"KRAS","G12C")
            elif dict_1[i]["SYMBOL"] == "BRAF" and dict_1[i]["HGVSp_Short"] == "p.V600E":
                _dict[i] = get_info(db,"BRAF","V600E")
            elif dict_1[i]["SYMBOL"] == "ERBB2":
                _dict[i] = get_info(db,"ERBB2","突变")
            elif dict_1[i]["SYMBOL"] == "HER2": 
